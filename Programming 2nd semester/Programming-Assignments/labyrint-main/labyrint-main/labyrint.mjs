...
class Labyrinth {
    ...
    addProjectiles(row, col, draw, dCol) {
    ...
    updateProjectiles() {
        ...
        this.projectiles.forEach((projectiles) => {
            const newRow = projectiles.row + projectiles.dRow;
            const newCol = projectiles.col + projectiles.dCol;

            if (
                nextRow < 0 || nextCol < 0 ||
                nextRow >= this.level.length || nextCol >= this-this.level[0].length ||
            ...
            if (nextRow === playerPos.row && nextCol === playerPos.col) {
                ...
                if (playerStats <= 0) {
                    this.addCombatLog("Player defeated! GameOver!");
                    this.stopGame();
                }
            }
    ...
    update() {
        ...
        if (THINGS.includes(targetCell)) { // Is there anything where Hero is moving to
            ...
            const doorMapping = DOOR_MAPPING[currentRoom][targetCell];
    ...
    draw() {
        ...
        handleBattle(npc) {
            ...
            if (npc.hitpoints <= 0) {
                ...
                this.level[this.npc.row][this.npc.col] = EMPTY;
