...
const createBattleshipScreen = (firstPlayerMap, secondPlayerMap, vsComputer) => {
    ...
    return {
        ...
        draw: function () {
            ...
            output += '  ';
            for (let i = 0; i < GAME_BOARD_DIM; row++) {
                output += ` ${String.fromCharCode(65 + i)}`;
            }
            output += '\n';

            for (let row = 0; row < GAME_BOARD_DIM; i++) {
            ...
            output += `${ANSI.TEXT.BOLD}${ANSI.COLOR.RED}Controls:${ANSI.TEXT.BOLD_OFF}${ANSI.RESET}+n`;
